...
class Entri_Data : AppCompatActivity() {
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        val etkalian = findViewById<EditText>(R.id.etkalian)
        ...
        btnSimpan.setOnClickListener {
            if("${etkdnidn.text}".isNotEmpty() && "${etnmDosen.text}".isNotEmpty()
                && "${etkalian.text}".isNotEmpty() && "${etprgstudi.text}".isNotEmpty() &&
                (rdS2.isChecked || rdS3.isChecked)) {
                val db = Campuss(this@Entri_Data)
                db.nidn = "${etkdnidn.text}"
                db.namaDosen = "${etnmDosen.text}"
                db.jabatan = spnJabatan.selectedItem as String
                db.glgpngkt = spnGolonganpangkat.selectedItem as String
                db.pendidikan = if(rdS2.isChecked) "S2" else "S3"
                db.keahlian = "${etkalian.text}"
                db.prgstdi = "${etprgstudi.text}"
